...
class LoginActivity : AppCompatActivity() {
    ...
    public override fun onStart() {
        ...
        if (currentUser != null) {
            // If the user is already signed in, navigate to the appropriate home page
            navigateToHomePage(currentUser.uid)
        }
    }

    private fun navigateToHomePage(uid: String) {
        // Access the Firebase database to retrieve the user's role
        ...
        userDatabaseRef.get().addOnCompleteListener { task ->
            if (task.isSuccessful) {
                ...
                if (userRole == "Provider") {
                    ...
                    // If the role is not recognized, log the error
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // Set OnClickListener for the "New User?" text view to navigate to RegisterActivity
